...
class AssignDriversForRegularRides extends Command
{
    ...
    public function handle()
    {
        ...
        Log::info('CANCEL REQUEST FROM COMMAND ARTISAN');
        ...
        Log::info("Start Check Uncompleted Requests");
        if ($uncompleted_requests) {
            foreach ($uncompleted_requests as $uncompleted_request) {
                ...
                if (isset($uncompleted_requests->driver_id)) {
                 	Log::info("Lets see the driver id => " . $uncompleted_request->driver_id);
        ...
        Log::info('Requests Count => ' . $requests->count());
        if ($requests->count() == 0) {
