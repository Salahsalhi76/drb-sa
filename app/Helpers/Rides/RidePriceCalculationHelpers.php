...
trait RidePriceCalculationHelpers
{
    ...
    protected function calculateBillForARide($pick_lat, $pick_lng, $drop_lat, $drop_lng, $total_distance, $duration, $zone_type, $type_prices, $coupon_detail, $timezone, $user_id, $waiting_time, $request_detail, $driver)
    {
        ...
        // application of discount amount
        if ($coupon_detail) {
            if ($coupon_detail->minimum_trip_amount < $sub_total) {
                ...
                if ($coupon_detail->maximum_discount_amount > 0 && $discount_amount > $coupon_detail->maximum_discount_amount) {
                    // update discount amount to the max amount
                ...
                if ($request_detail) {
                    ...
                    Log::info('Create user promo');
                    // save new promo user row
                    $promo = new PromoUser();
                    $promo->promo_code_id = $coupon_detail->id;
                    $promo->user_id = $request_detail->user_id;
                    $promo->request_id = $request_detail->id;

                    if ($coupon_detail->promo_type === "squential") {
                        $promo->time_left =  $coupon_detail->sequential_times;
                    }
                ...
                //
                if ($request_detail) {
                    $promoUsers =  PromoUser::where('request_id', $request_detail->id)->whereNull('time_left')->delete();
                }
            }
        } else {
            ...
            // block of code
            if ($request_detail) {
                $existing_promo_user = PromoUser::where('user_id', $request_detail->userDetail->id)->where('time_left', ">", 0)->first();
                if ($existing_promo_user) {

                    $promo_code_detail = $existing_promo_user->promo;

                    if ($promo_code_detail->promo_type === "squential") {


                        $promo_code_time_left = $existing_promo_user->time_left;

                        $sequential_times = $promo_code_detail->sequential_times;

                        $sequential_times_discount = $promo_code_detail->discount_sequential;

                        $index_discount = ($sequential_times - $promo_code_time_left) + 1;

                        $discount_amount = $sub_total * ($sequential_times_discount[$index_discount] / 100);

                        if ($promo_code_detail->maximum_discount_amount > 0 && $discount_amount > $promo_code_detail->maximum_discount_amount) {
                            // update discount amount to the max amount
                            $discount_amount = $promo_code_detail->maximum_discount_amount;
                        }
                        ...
                        $existing_promo_user->update([
                            'time_left' => $existing_promo_user->time_left - 1
                        ]);
                    }
                }
            }
        ...
        //  trip details calculation of cancelation fee
        if ($request_detail) {
        ...
        $tax_amount = ($coupon_applied_sub_total * ($tax_percent / 100));
        ...
        if ($admin_commision_type == 1) {
            ...
            $admin_commision = ($coupon_applied_sub_total * ($service_fee / 100));
        ...
        $driver_commision = $coupon_applied_sub_total + $discount_amount;

        // Driver Commission
        if ($coupon_detail && $coupon_detail->deduct_from == 2) {
            $driver_commision = $coupon_applied_sub_total;
        }
        ...
        // Total Amount
        $total_amount = $coupon_applied_sub_total + $admin_commision_with_tax;

        Log::info('Coupon suplied dicount => ' . $coupon_applied_sub_total);
        Log::info('Discount total price ' . $discounted_total_price);
        Log::info('Total Price => ' . $total_amount);
        Log::info('Discount Amount => ' . $discount_amount);
        Log::info('Sub Total => ' . $sub_total);
        Log::info('New Key Of The Base Price => ' . $total_amount + $discount_amount);

        $basic_total_price = $total_amount + $discount_amount;
        ...
        $basic_total_price  = floor($basic_total_price);
        ...
        if ($request_detail) {
            ...
            return $result = [
                ...
                'basic_total_price' => $basic_total_price
            ];
        }
