...
class CancellationReasonsController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($app_for == 'taxi' || $app_for == 'delivery') {
            $query = $this->cancellation_reasons->where('user_type', $user_type)->where('cancellation_type', 'after_driver_accept_ride');
        } else {
            ...
            })->where('cancellation_type', 'after_driver_accept_ride');
        }
    ...
    public function cancellationUserIndex()
    {
        $user_type = access()->hasRole(Role::USER) ? 'user' : 'driver';
        $query = $this->cancellation_reasons->where('user_type', $user_type)->where('cancellation_type', 'before_driver_accept_ride');
