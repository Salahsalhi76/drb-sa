...
class PaymentController extends BaseController
{
    ...
    {
        if (access()->hasRole(Role::USER)) {
            $query = UserWalletHistory::where('user_id', auth()->user()->id)->select('*', DB::raw('FORMAT(amount, 2) as amount'));
            ...
            $wallet_balance = number_format((float) $user_wallet->amount_balance, 2);
            ...
            $query = DriverWalletHistory::where('user_id', auth()->user()->driver->id)->select('*', DB::raw('FORMAT(amount, 2) as amount'))->orderBy('created_at', 'desc');
            ...
            $query = OwnerWalletHistory::where('user_id', auth()->user()->owner->id)->select('*', DB::raw('FORMAT(amount, 2) as amount'))->orderBy('created_at', 'desc');
        ...
        $payment_gateways = array(
            'flutter_wave' => array(
                'enabled' => $enable_flutter_wave,
                'image' => $flutterwave_image,
                'url' => $url . "/flutterwave"
            ),

            'mercadepago' => array(
                'enabled' => $enable_mercadopago,
            ...
            'paystack' => array(
                'enabled' => $enable_paystack,
                'image' => $paystack_image,
                'url' => $url . "/paystack"
            ),

            'razorpay' => array(
                'enabled' => $enable_razor_pay,
                'image' => $Razor_image,
                'url' => $url . "/razorpay"
            ),

            'stripe' => array(
                'enabled' => $enable_stripe,
                'image' => $stripe_image,
                'url' => $url . "/stripe"
            ),

            'khalti' => array(
                'enabled' => $enable_khalti,
                'image' => $khalti_image,
                'url' => $url . "/khalti"
            ),
            /*cashfree*/
        ...
        Log::info('Wallet History balance => ' . $wallet_balance);

        return response()->json([
            'success' => true,
            'message' => 'wallet_history_listed',
            'wallet_balance' => number_format((float) $wallet_balance, 2),
    ...
    public function transferMoneyFromWallet(Request $request)
    {
        ...
        if ($request->mobile == $user->mobile && $invalid_mobile) {
            ...
            //Throw exception
            $this->throwCustomException('الرقم خطاء او ادخل الرقم بدون 0 مثال 5310');
        }
        ...
        if ($user_wallet && $user_wallet->amount_balance < $minimum_wallet_amount) {
            ...
            //Throw exception
            $this->throwCustomException('الرصيد غير كافي لتحويل الأموال');
        }
