    ...
    */
     ...
     * */
     ...
     public function respondForBid(ValidatorRequest $request)
     {
         ...
         try {
             // 1. بداية العملية - التحقق من التفاصيل
             Log::info("{$logPrefix} Initiating process for bid response", [
                 'action' => 'start',
             ...
             $driver->save();
             Log::info("{$logPrefix} Driver status updated", [
                 'driver_id' => $driver->id,
                 'new_status' => $driver->available
             ]);
             ...
             Log::info("{$logPrefix} Push notification dispatched to driver", [
                 'driver_id' => $driver->id,
                 'title' => $title,
                 'body' => $body
             ]);
             ...
             return $this->respondError('Failed to process bid response: ' . $e->getMessage(),500);
         }
     }
