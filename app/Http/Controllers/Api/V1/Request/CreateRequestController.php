        ...
        foreach ($nearest_drivers[0] as $driver) {
            ...
            $selected_drivers[$i]["updated_at"] = date('Y-m-d H:i:s');
                ...
                if (get_settings('trip_dispatch_type') == 0) {
                    ...
                    try {
                        // 1. تسجيل بدء عملية الإرسال
                        Log::channel('notifications')->info("Initiating notification process", [
                            'driver_id' => $driver->id,
                            'request_id' => $request_detail->id,
                            'step' => 'start'
                        ]);
                
                        // 2. تحديث Firebase مع التحقق من النجاح
                        $firebaseUpdate = $this->database->getReference('request-meta/'.$request_detail->id)
                            ->set([
                                'driver_id' => $driver->id,
                                'request_id' => $request_detail->id,
                        ...
                        if (!$firebaseUpdate) {
                            throw new \Exception("Failed to update Firebase");
                        }
                
                        // 3. الحصول على بيانات السائق مع التحقق من الوجود
                        $driver = Driver::with('user')->find($driver->id);
                        if (!$driver || !$driver->user) {
                            throw new \Exception("Driver or user record not found");
                        }
                
                        $notifiableDriver = $driver->user;
                        
                        // 4. إعداد محتوى الإشعار مع الترجمات الاحتياطية
                        $title = trans('push_notifications.new_request_title', [], $notifiableDriver->lang) ?: 'New Trip Request';
                        $body = trans('push_notifications.new_request_body', [], $notifiableDriver->lang) ?: 'You have a new trip request';
                        
                        $pushData = [
                            'title' => $title,
                            'message' => $body,
                            'push_type' => 'meta-request',
                            'request_id' => $request_detail->id,
                            'driver_id' => $driver->id,
                            'sent_at' => now()->toDateTimeString()
                        ];
                
                        // 5. إرسال الإشعار مع تتبع الحالة
                        $notificationJob = new SendPushNotification(
                            $notifiableDriver,
                            $title,
                            $body,
                            $pushData
                        ...
                        dispatch($notificationJob)
                            ->onQueue('high_priority_notifications')
                            ->delay(now()->addSeconds(1)); // تأخير بسيط لتجنب التحميل الزائد
                
                        // 6. تحديث حالة الإشعار في Firebase
                        $this->database->getReference('request-meta/'.$request_detail->id.'/notification_status')
                            ->set('dispatched');
                
                        // 7. تسجيل نجاح العملية
                        Log::channel('notifications')->info("Notification dispatched successfully", [
                            'driver_id' => $driver->id,
                            'request_id' => $request_detail->id,
                            'fcm_token' => substr($notifiableDriver->fcm_token, 0, 10).'...', // تسجيل جزء من التوكن فقط
                            'platform' => $notifiableDriver->device_type,
                            'step' => 'completed'
                        ]);
                
                    } catch (\Exception $e) {
                        // 8. تسجيل الخطأ بالتفصيل
                        Log::channel('notifications')->error("Notification failed", [
                            'driver_id' => $driver->id ?? 'unknown',
                            'request_id' => $request_detail->id ?? 'unknown',
                            'error' => $e->getMessage(),
                        ...
                        // 9. تحديث حالة الفشل في Firebase
                        $this->database->getReference('request-meta/'.$request_detail->id.'/notification_status')
                            ->set('failed: '.$e->getMessage());
                    }
                }
