...
<script>
  ...
  requestRef.on("child_added", (snapshot) => {
    if(shouldProcessChildAdded){
      ...
      var assign_driver='';
      if(!snapshot.hasOwnProperty('driver_id')){
          assign_driver+=`<a class="dropdown-item" id="assign_${snapshot.request_id}" href="${baseUrl+'/dispatch/assign/'+snapshot.request_id}">
                            <i class="fa fa-dot-circle-o"></i>@lang('view_pages.assign_driver')</a>`;
      }
    ...
    requestRef.once('value').then(function(snapshot){
      ...
      requests[i].forEach(function (request) {
        ...
        if(request.is_accept == 1){
          $('#assign_'+request.request_id).remove();
        ...
        var assign_driver = '';
        if(!request.hasOwnProperty('driver_id')){
            assign_driver =`<a class="dropdown-item" id="assign_${request.request_id}" href="${baseUrl+'/dispatch/assign/'+request.request_id}">
              <i class="fa fa-dot-circle-o"></i>@lang('view_pages.assign_driver')</a>`;
        }
        html_data += `
